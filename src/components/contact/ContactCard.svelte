  ...
  <div class="relative h-full bg-gradient-to-br from-[#1a1a2e] to-[#16213e] rounded-lg p-6 shadow-xl border border-[#ff3d00]/20 transition-all duration-300 group-hover:border-[#ff3d00]/40">
    ...
    <div class="relative">
      {#if href}
        <a
          {href}
          class="block text-lg text-white font-medium no-underline transition-colors duration-300 hover:text-[#ff8a00] focus:outline-none focus:text-[#ff8a00] group/link"
        >
          <slot />
          <div class="absolute -right-2 top-1/2 -translate-y-1/2 opacity-0 transform translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0">
            <!-- Solar wind effect -->
            <div class="absolute inset-0 bg-gradient-to-l from-[#ff3d00]/20 via-transparent to-transparent w-12 h-full -translate-x-full group-hover/link:translate-x-0 transition-transform duration-500"></div>
            <svg
              class="w-5 h-5 text-[#ff3d00] relative z-10"
              viewBox="0 0 24 24"
              fill="none"
