<script lang="ts">
  ...
  function initParticles() {
    ...
    const text = title;
    ctx.font = 'bold 24px "Satoshi", "Source Sans Pro", sans-serif';
    ctx.fillStyle = "white";
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    const x = Math.max(
      ...
      Math.min(
        1,
        (mouseX - strictBounds.left) / (strictBounds.right - strictBounds.left),
      ),
    );
    const y = Math.max(
      0,
      Math.min(
        1,
        (mouseY - strictBounds.top) / (strictBounds.bottom - strictBounds.top),
      ),
    );
  ...
  function handleMouseLeave(event?: MouseEvent) {
    ...
    const resetAnimation = () => {
      ...
      if (
        Math.abs(magnetStrength.x) < 0.01 &&
        Math.abs(magnetStrength.y) < 0.01
      ) {
  ...
  onMount(() => {
    ...
    cardElement
      .querySelectorAll('*:not(a, button, [role="button"])')
      .forEach((el) => {
...
>
  ...
  >
    ...
    <!-- Glass card -->
    <div
      class="absolute inset-0 rounded-3xl backdrop-blur-sm bg-white/[0.02] border border-white/[0.05]"
    >
      <!-- Shine effect -->
      <div
        class="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700"
      >
        <div
          class="absolute inset-0 bg-gradient-to-r from-transparent via-white/[0.05] to-transparent -skew-x-12 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000"
        ></div>
      </div>
    </div>
    ...
    >
      ...
      <div class="space-y-6">
        <div class="relative w-16 h-16">
          <!-- Glowing orb -->
          <div
            class="absolute inset-0 rounded-full bg-gradient-to-br from-[#ff3d00] to-[#ff8a00] blur-lg opacity-50 animate-pulse"
          ></div>
          <div
          ...
          >
            ...
            >
              <div
                class="w-8 h-8 rounded-full bg-gradient-to-br from-[#ff3d00] to-[#ff8a00] transform group-hover:scale-110 transition-transform duration-300"
              ></div>
            </div>
          </div>
        </div>
      ...
      <div class="space-y-6">
        ...
        <div class="flex items-center gap-3 text-[#ff3d00] font-medium">
          <span class="relative">
            Explore
            <div
              class="absolute inset-x-0 h-px -bottom-1 bg-gradient-to-r from-[#ff3d00] to-[#ff8a00] transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"
            ></div>
          </span>
          <svg
