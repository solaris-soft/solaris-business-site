<script lang="ts">
	...
	const fragmentShaderSource = `
		...
		vec3 plasma(vec2 uv, float t) {
			// Simplified base plasma effect
			float v1 = sin((uv.x + t * 0.1) * 8.0);
			float v2 = cos((uv.y + t * 0.15) * 6.0);
			float v3 = sin(sqrt(pow(uv.x - 0.5, 2.0) + pow(uv.y - 0.5, 2.0)) * 10.0);
			
			// Simplified flow field
			vec2 flow = vec2(
				sin(uv.y * 3.0 + t * 0.2) * 0.3,
				cos(uv.x * 3.0 + t * 0.15) * 0.3
			);
			
			// Reduced number of effects
			float noise1 = fbm(uv * 2.0 + flow + t * 0.08) * 1.2;
			float noise2 = fbm(uv * 3.0 - flow * 0.5 + t * 0.12) * 1.2;
			
			// Combine effects
			float plasma = v1 + v2 + v3 + noise1 * 0.8 + noise2 * 0.4;
			
			// Simplified color palette
			vec3 col1 = vec3(0.7 + 0.3 * sin(t * 0.1), 0.3, 0.1);
		...
		void main() {
			...
			float vignette = smoothstep(1.2, 0.4, length((uv - 0.5) * 1.8));
			color *= vignette;
			
			// Adjusted gamma and tone mapping
					...
					>
						...
						>
							<span 
								...
								aria-hidden="true"
								style="
									background: linear-gradient(to bottom, rgba(255,255,255,0.9) 0%, rgba(255,215,192,0.9) 100%);
									-webkit-background-clip: text;
									background-clip: text;
									color: transparent;
									filter: blur(8px);
